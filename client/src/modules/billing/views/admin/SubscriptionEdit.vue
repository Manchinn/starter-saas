  ...
  <AppLayout>
    <div class="space-y-6">
      <PageHeader
        ...
        :subtitle="t('billing.editSubscriptionDesc')"
        :back-to="`/admin/billing/subscriptions/${orgId}`" />

      <LoadingSpinner v-if="loading" padding="md" />

      <form v-else class="card-md p-6 space-y-5 max-w-xl" @submit.prevent="save">
        <div>
          <label class="label">{{ t('billing.colPlan') }}</label>
        ...
        <div>
          <label class="label">{{ t('billing.statusField') }}</label>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            <label class="label">{{ t('billing.startsAt') }}</label>
            <input v-model="form.currentPeriodStart" type="date" class="input" />
          </div>
          <div>
            <label class="label">{{ t('billing.periodEnd') }}</label>
            <input v-model="form.currentPeriodEnd" type="date" class="input" />
          </div>
        </div>
        ...
        <div class="flex items-center gap-3 pt-2">
          <button type="submit" :disabled="saving" class="btn-primary disabled:opacity-50">{{ t('common.save') }}</button>
          <RouterLink :to="`/admin/billing/subscriptions/${orgId}`" class="btn-ghost">{{ t('common.cancel') }}</RouterLink>
        </div>
      </form>
    </div>
  </AppLayout>
