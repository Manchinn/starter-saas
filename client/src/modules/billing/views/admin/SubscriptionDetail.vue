  ...
  <AppLayout>
    <div class="space-y-6">
      <PageHeader
        ...
        <template #subtitle>
          <span v-if="sub">{{ planName }} · {{ t('billing.createdAt') }} {{ formatDate(sub.createdAt) }}</span>
        </template>
        <template #badge>
      ...
      <div v-else-if="!sub" class="card-md text-center py-14 text-slate-400">
        {{ t('billing.notFound') }}
      </div>
          ...
          <div class="card overflow-hidden">
            ...
            <table class="w-full text-sm">
              <thead class="bg-[#F7F9FC] border-b border-[#E2E8F0]">
                <tr class="text-left">
                  <th class="th">{{ t('billing.colInvoice') }}</th>
                  <th class="th text-right">{{ t('billing.colAmount') }}</th>
                  <th class="th">{{ t('billing.colGateway') }}</th>
              ...
              <tbody class="divide-y divide-slate-50">
                ...
                <tr v-for="inv in invoices" :key="inv.id" class="hover:bg-[#F7F9FC]">
                  ...
                  <td class="px-5 py-3 text-right">
                    <button @click="viewInvoice = inv" class="p-1.5 text-slate-400 hover:text-primary-600 hover:bg-slate-100 rounded">
        ...
        <div class="space-y-6">
          ...
          <div class="card p-5">
            ...
            <div class="space-y-2.5">
              <button @click="toggleSuspend" :disabled="busy" class="btn-secondary w-full justify-center disabled:opacity-50">
                <PauseCircleIcon class="w-4 h-4" />
                {{ sub.suspended ? t('billing.resume') : t('billing.suspend') }}
              </button>
              <button @click="cancelSub" :disabled="busy || sub.status === 'canceled'" class="btn-secondary w-full justify-center disabled:opacity-50">
    ...
    <div v-if="viewInvoice" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40" @click.self="viewInvoice = null">
      <div class="card-md w-full max-w-md p-6 space-y-4">
        <div class="flex items-center justify-between">
          <h3 class="font-semibold text-slate-800">{{ t('billing.invoiceDetails') }}</h3>
          <button @click="viewInvoice = null" class="p-1 text-slate-400 hover:text-slate-700"><XMarkIcon class="w-5 h-5" /></button>
        </div>
...
const intervalLabel = computed(() => (plan.value ? t('billing.interval.' + plan.value.interval) : ''))
...
const statusLabel = computed(() =>
  sub.value?.suspended ? t('billing.suspended') : t('billing.status.' + sub.value.status))
...
async function toggleSuspend() {
  ...
  error.value = ''
  try {
    sub.value = { ...sub.value, ...(await store.suspendSubscription(orgId, next)) }
  } catch (err) {
...
async function cancelSub() {
  ...
  error.value = ''
  try {
    sub.value = { ...sub.value, ...(await store.cancelSubscription(orgId, true)) }
  } catch (err) {
    error.value = err.response?.data?.message || t('billing.saveFailed')
  } finally {
    busy.value = false
