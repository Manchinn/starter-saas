  ...
  <AppLayout>
    <div class="space-y-5">
      <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
        ...
        <select v-model="statusFilter" @change="load" class="input w-auto py-1.5 text-sm">
        ...
        <table class="w-full text-sm">
          <thead class="bg-[#F7F9FC] border-b border-[#E2E8F0]">
            <tr class="text-left">
              ...
              <th class="th">{{ t('billing.requestedPlan') }}</th>
              <th class="th">{{ t('billing.colStatus') }}</th>
              <th class="th">{{ t('billing.requestedOn') }}</th>
              <th class="th">{{ t('billing.decided') }}</th>
          ...
          <tbody class="divide-y divide-slate-50">
            ...
            <tr v-for="r in store.adminPlanRequests" :key="r.id" @click="goTo(r.organizationId)"
              ...
              <td class="px-5 py-3"><span :class="['badge', statusTone(r.status)]">{{ t('billing.requestStatus.' + r.status) }}</span></td>
              <td class="px-5 py-3 text-slate-500">{{ formatDate(r.createdAt) }}</td>
              <td class="px-5 py-3 text-slate-500">
                <template v-if="r.decidedAt">
                  {{ formatDate(r.decidedAt) }}
                  <span v-if="r.decider" class="text-xs text-slate-400">· {{ r.decider.name }}</span>
