      ...
      <div class="grid grid-cols-1 xl:grid-cols-3 gap-5">
        ...
        <section class="card overflow-hidden xl:col-span-2">
          <header class="px-5 py-4 border-b border-[#E2E8F0] flex items-center justify-between gap-3">
            <div class="flex items-center gap-3 min-w-0">
              <div class="w-1 h-4 bg-primary-500 flex-shrink-0"></div>
              <h3 class="text-[14px] font-semibold text-[#1C2434] truncate">{{ t('billing.subscriptions') }}</h3>
            </div>
            <RouterLink
              to="/admin/billing/subscriptions"
              class="text-[12.5px] font-medium text-primary-600 hover:text-primary-700 flex items-center gap-1 whitespace-nowrap"
            >
              {{ t('dashboard.seeAll') }}
              <ArrowRightIcon class="w-3.5 h-3.5" />
            </RouterLink>
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-sm">
              <thead class="bg-[#F7F9FC] border-b border-[#E2E8F0]">
                <tr class="text-left">
                  <th class="th">{{ t('billing.organization') }}</th>
                  <th class="th">{{ t('billing.colPlan') }}</th>
                  <th class="th">{{ t('billing.colStatus') }}</th>
                  <th class="th">{{ t('billing.periodEnd') }}</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-50">
                ...
                <tr
                  ...
                  :key="s.id"
                  class="hover:bg-[#F7F9FC]/60 transition-colors cursor-pointer"
                  @click="goToSub(s.organizationId)"
                >
                  <td class="td">
                  ...
                  <td class="td">
                    <span :class="['badge', subTone(s)]">
                      {{ s.suspended ? t('billing.suspended') : t('billing.' + s.status) }}
                    </span>
                  </td>
