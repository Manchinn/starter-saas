  ...
  <div class="bg-white border border-[#E2E8F0] p-6 space-y-4">
    ...
    <ol v-else class="relative">
      <li v-for="(l, idx) in logs" :key="l.id" class="relative pl-8 pb-5">
        <span class="absolute left-2.5 top-2 -translate-x-1/2 w-2.5 h-2.5 bg-primary-500 ring-2 ring-white shadow"></span>
        ...
        <div class="flex items-baseline justify-between gap-2">
          <p class="text-sm font-medium text-[#1C2434]">
            <code class="font-mono text-[11px] bg-[#F1F5F9] text-[#374151] px-1.5 py-0.5">{{ l.action }}</code>
          </p>
