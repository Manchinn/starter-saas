        ...
        <table class="w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-200 text-left">
            <tr>
              ...
              <th class="px-5 py-3 font-medium text-gray-600">Created</th>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-else-if="!items.length">
              <td colspan="5" class="text-center py-12 text-gray-400">No items found.</td>
            </tr>
            <tr
              v-for="item in items"
              :key="item.id"
            ...
            >
              <td class="px-5 py-3 font-medium text-gray-900">{{ item.title }}</td>
              <td class="px-5 py-3 text-gray-500 max-w-xs truncate">{{ item.description || '—' }}</td>
              <td class="px-5 py-3">
                <span
              ...
              <td class="px-5 py-3 text-gray-500 text-xs">{{ fmtDate(item.createdAt) }}</td>
              <td class="px-5 py-3 text-right whitespace-nowrap">
                <RouterLink
                  v-can="'items.edit'"
                  :to="`/items/${item.id}/edit`"
                  class="text-primary-600 hover:underline text-xs mr-3"
                >Edit</RouterLink>
                <button
                  v-can="'items.delete'"
...
async function fetchItems() {
  loading.value = true
  try {
    const { data } = await api.get('/items', { params: { page: page.value, limit, search: search.value } })
