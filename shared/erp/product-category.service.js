...
const create = async ({ name, description, parentId, status = 'active' }) => {
  ...
  return ProductCategory.create({ name: name.trim(), description, parentId: parentId || null, status })
}

const update = async (id, { name, description, parentId, status }) => {
  const cat = await ProductCategory.findByPk(id)
  if (!cat) throw { status: 404, message: 'Product category not found' }
