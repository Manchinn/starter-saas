        ...
        <table class="w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-200 text-left">
            <tr>
              ...
              <th class="px-5 py-3 font-medium text-gray-600">Supplier</th>
            ...
            <tr v-for="row in rows" :key="row.id" class="hover:bg-gray-50 transition">
              ...
              <td class="px-5 py-3 text-right">
                <div class="flex items-center justify-end gap-2">
                  <RouterLink :to="`/erp/good-receive/${row.id}`" class="text-xs px-3 py-1 border rounded hover:bg-gray-50">View</RouterLink>
...
async function approveRow(row) {
  ...
  try {
    await api.post(`/erp/good-receive/${row.id}/confirm`)
    load()
  } catch (err) {
...
async function confirmDelete(row) {
  ...
  try {
    await api.delete(`/erp/good-receive/${row.id}`)
    load()
  } catch (err) {
