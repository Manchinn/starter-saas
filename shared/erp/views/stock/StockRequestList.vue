  ...
  <AppLayout>
    <div class="space-y-6">
      <div class="flex items-center justify-between gap-4">
        <h1 class="text-2xl font-bold text-gray-900">Stock Requests</h1>
        ...
        <table class="w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-200 text-left">
            <tr>
              ...
              <th class="px-5 py-3 font-medium text-gray-600">From Store</th>
              <th class="px-5 py-3 font-medium text-gray-600">To Store</th>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-for="row in rows" :key="row.id" class="hover:bg-gray-50 transition">
              ...
              <td class="px-5 py-3">
                <span class="text-gray-700 font-medium">{{ row.fromStore?.name || '—' }}</span>
              </td>
              <td class="px-5 py-3">
                <span class="text-gray-700 font-medium">{{ row.toStore?.name || '—' }}</span>
              </td>
              <td class="px-5 py-3">
              ...
              <td class="px-5 py-3 text-right">
                <div class="flex items-center justify-end gap-2">
                  <RouterLink :to="`/erp/stock-request/${row.id}`" class="text-xs px-3 py-1 border rounded hover:bg-gray-50">View</RouterLink>
...
async function approveRow(row) {
  ...
  try {
    await api.post(`/erp/stock-request/${row.id}/confirm`)
    load()
  } catch (err) {
...
async function confirmDelete(row) {
  ...
  try {
    await api.delete(`/erp/stock-request/${row.id}`)
    load()
  } catch (err) {
