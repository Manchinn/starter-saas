        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-if="!items.length">
              <td colspan="7" class="px-3 py-10 text-center text-gray-400">No items yet. Click "+ Add Item" to start.</td>
            </tr>
          ...
          <tfoot v-if="items.length" class="border-t-2 border-gray-200 bg-gray-50">
            <tr>
              <td colspan="3" class="px-3 py-2 text-xs font-medium text-gray-500">Totals</td>
...
onMounted(async () => {
  try {
    const [prodRes, storeRes, uomRes] = await Promise.all([
