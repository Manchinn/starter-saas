  ...
  <AppLayout>
    <div class="max-w-2xl space-y-6">
      ...
      <div v-else-if="notFound" class="bg-red-50 text-red-700 text-sm px-4 py-3 rounded-lg">
        Customer not found. <RouterLink to="/erp/customers" class="underline ml-1">Back to list</RouterLink>
      </div>
      ...
      <div v-else class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        <div class="flex justify-between items-center pt-2">
          <button v-can="'erp.customers.delete'" @click="confirmDelete" class="px-4 py-2 text-sm text-red-500 border border-red-200 rounded-lg hover:bg-red-50 transition">Delete Customer</button>
...
const form     = ref({ name: '', company: '', email: '', phone: '', address: '', notes: '', status: 'active' })
...
onMounted(async () => {
  try {
    ...
    form.value = { name: c.name, company: c.company || '', email: c.email || '', phone: c.phone || '', address: c.address || '', notes: c.notes || '', status: c.status }
  } catch {
    notFound.value = true
  } finally {
    loading.value = false
...
async function save() {
  ...
  saving.value = true
  try {
    await api.put(`/erp/customers/${route.params.id}`, form.value)
