...
const { selectedIndex: selectedRowIndex, shortcuts, open: openRow } = useListShortcuts({
  rows: customers, page, totalPages,
  open:        c => router.push(`/erp/customers/${c.id}/edit`),
  create:      () => router.push('/erp/customers/create'),
...
const columns = [
  ...
  columnHelper.display({
    ...
    cell: info => info.row.original.group
      ? h('span', { class: 'inline-flex items-center px-2 py-0.5 bg-blue-50 text-blue-700 text-xs font-medium' }, info.row.original.group.name)
      : h('span', { class: 'text-[#9BA7B0] text-xs' }, '—'),
  }),
  columnHelper.display({
    id: 'contact',
