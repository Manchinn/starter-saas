...
function onKeydown(e) {
  ...
  if (e.key === 'ArrowDown') {
    e.preventDefault()
    selectedRowIndex.value = Math.min(selectedRowIndex.value + 1, uoms.value.length - 1)
    ...
    const u = uoms.value[selectedRowIndex.value]
    if (u) router.push(`/erp/uom/${u.id}/edit`)
  } else if (e.shiftKey && e.key === 'C') {
    e.preventDefault()
    router.push('/erp/uom/create')
