      ...
      <div class="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden">
        ...
        <div class="px-5 py-3 border-b border-[#E2E8F0] flex items-center gap-3">
          <div class="relative flex-1 min-w-0">
            ...
            <input v-model="search" @input="onSearch" type="search" :placeholder="t('erp.uom.searchPh')"
...
import { PlusIcon, MagnifyingGlassIcon, PencilIcon, TrashIcon, ScaleIcon, AdjustmentsHorizontalIcon, XMarkIcon } from '@heroicons/vue/24/outline'
...
const columns = [
  columnHelper.accessor('abbreviation', {
    header: () => t('erp.uom.colCode'),
    cell: info => h('span', { class: 'font-mono text-sm font-bold text-[#374151]' }, info.getValue()),
  }),
  columnHelper.accessor('name', {
    header: () => t('erp.uom.colName'),
    cell: info => info.getValue() || '—',
  }),
  columnHelper.accessor('description', {
    header: () => t('erp.uom.colDesc'),
    cell: info => info.getValue() || '—',
  }),
  columnHelper.accessor('status', {
    header: () => t('erp.uom.colStatus'),
    cell: info => {
  ...
  columnHelper.display({
    ...
    cell: info => h('div', { class: 'flex items-center justify-end gap-1' }, [
      h(RouterLink, {
        to: `/erp/uom/${info.row.original.id}/edit`,
