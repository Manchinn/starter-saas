      ...
      <div v-if="formModal.open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6 space-y-5">
          <div class="flex items-center justify-between">
            <h2 class="text-base font-semibold text-gray-900">Edit Price List</h2>
      ...
      <div v-if="applyModal.open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-2xl flex flex-col max-h-[80vh]">
          <div class="flex items-center justify-between px-6 pt-5 pb-3 border-b border-gray-100">
            <div>
              <h2 class="text-base font-semibold text-gray-900">Apply Price List to Order Items</h2>
              <p class="text-xs text-gray-500 mt-0.5">
                Price List: <span class="font-medium text-gray-700">{{ applyModal.pricing?.name }}</span>
...
const form = reactive({ code: '', name: '', description: '', unitPrice: 0, currency: 'USD', status: 'active', orderItemId: '', customerGroupId: '' })
...
async function saveForm() {
  ...
  formModal.error  = ''
  try {
    const payload = { ...form, orderItemId: form.orderItemId || null, customerGroupId: form.customerGroupId || null }
