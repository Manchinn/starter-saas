      ...
      <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-5">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input v-model="form.code" type="text" placeholder="e.g. PL-001"
          ...
          <div>
            ...
            <select v-model="form.orderItemId" :disabled="orderItemsLoading"
              class="w-full px-3 py-2 border rounded-lg text-sm bg-white focus:outline-none focus:ring-2 focus:ring-primary-500">
              <option value="">— None —</option>
...
onMounted(async () => {
  const [groupsRes] = await Promise.allSettled([api.get('/api/erp/customer-groups/all')])
...
async function save() {
  ...
  saving.value = true
  try {
    await api.post('/api/erp/pricing', { ...form.value, orderItemId: form.value.orderItemId || null, customerGroupId: form.value.customerGroupId || null })
