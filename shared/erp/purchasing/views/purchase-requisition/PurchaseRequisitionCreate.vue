      ...
      <div v-if="vendorCreateOpen" class="fixed inset-0 z-50 flex">
        ...
        <div class="w-full max-w-md bg-white shadow-2xl overflow-y-auto flex flex-col">
          ...
          <div class="flex-1 px-6 py-5 space-y-4">
            <FormField name="vendorName" :label="t('erp.vendors.name') || 'Name'" :errors="{}" required>
              <template #default="{ id }">
                <input :id="id" v-model="newVendor.name" ref="newVendorNameRef" type="text"
                  placeholder="Vendor name" class="input" />
              </template>
            </FormField>
            <FormField name="vendorCode" :label="t('erp.vendors.code') || 'Code'" :errors="{}"
              v-model="newVendor.code" />
            <div class="grid grid-cols-2 gap-4">
              <FormField name="vendorEmail" :label="t('erp.vendors.email') || 'Email'" :errors="{}"
                v-model="newVendor.email" type="email" />
              <FormField name="vendorPhone" :label="t('erp.vendors.phone') || 'Phone'" :errors="{}"
                v-model="newVendor.phone" />
            </div>
            <FormField name="vendorAddress" :label="t('erp.vendors.address') || 'Address'" :errors="{}"
