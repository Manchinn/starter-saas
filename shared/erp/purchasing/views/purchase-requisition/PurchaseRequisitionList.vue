  ...
  <AppLayout>
    <div class="space-y-5">
      ...
      <div class="flex items-center justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold text-[#1C2434]">{{ t('erp.purchasing.title') }}</h1>
          <p class="text-sm text-[#637381] mt-0.5">
            <template v-if="!loading">{{ total.toLocaleString() }} {{ t('erp.purchasing.records') }}</template>
        ...
        <DataTable :columns="columns" :data="requisitions" :loading="loading" :total="total"
          ...
          <template #empty>
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <p class="text-sm font-medium text-[#637381]">{{ t('erp.purchasing.noFound') }}</p>
            </div>
          </template>
        </DataTable>
...
async function fetch() {
  loading.value = true
  try {
    const { data } = await api.get('/erp/purchasing/requisitions', {
...
async function confirmDelete(row) {
  ...
  try {
    await api.delete(`/erp/purchasing/requisitions/${row.id}`)
    fetch()
  } catch (err) {
...
const columns = [
  columnHelper.accessor('refNo', {
    header: () => t('erp.purchasing.colRefNo'),
    cell: info => h(RouterLink, {
      to: `/erp/purchasing/requisitions/${info.row.original.id}`,
      class: 'font-mono text-sm text-primary-600 hover:underline font-medium',
    }, () => info.getValue()),
  }),
  columnHelper.accessor('date', {
    header: () => t('erp.purchasing.colDate'),
    cell: info => h('span', { class: 'text-sm text-[#374151]' }, fmtDate(info.getValue())),
  }),
  columnHelper.accessor('requestedBy', {
    header: () => t('erp.purchasing.colRequestedBy'),
    cell: info => h('span', { class: 'text-sm text-[#637381]' }, info.getValue() || '—'),
  }),
  columnHelper.accessor('department', {
    header: () => t('erp.purchasing.colDepartment'),
    cell: info => h('span', { class: 'text-sm text-[#637381]' }, info.getValue() || '—'),
  }),
  columnHelper.display({
    id: 'vendor',
    header: () => t('erp.purchasing.colVendor'),
    cell: info => {
      const v = info.row.original.vendor
      return v ? h('span', { class: 'text-sm text-[#637381]' }, v.name) : h('span', { class: 'text-[#9BA7B0] text-xs' }, '—')
    },
  }),
  columnHelper.accessor('status', {
    ...
    cell: info => {
      ...
      const label = t(`erp.purchasing.status${s.charAt(0).toUpperCase() + s.slice(1)}`)
      return h('span', { class: `inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${cls}` }, label)
    },
  }),
  columnHelper.display({
    id: 'actions',
