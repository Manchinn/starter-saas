const svc = require('../services/receive-payment.service')
...
const getById = async (req, res, next) => {
  try {
    res.json({ data: { receivePayment: await svc.getById(req.params.id) } })
  } catch (err) { next(err) }
}
...
const create = async (req, res, next) => {
  try {
    ...
    res.status(201).json({ data: { receivePayment: doc } })
  } catch (err) { next(err) }
}

const confirm = async (req, res, next) => {
  try {
    res.json({ data: { receivePayment: await svc.confirm(req.params.id, req.user?.id) } })
  } catch (err) { next(err) }
}

const cancel = async (req, res, next) => {
  try {
    res.json({ data: { receivePayment: await svc.cancel(req.params.id, req.user?.id) } })
  } catch (err) { next(err) }
}
