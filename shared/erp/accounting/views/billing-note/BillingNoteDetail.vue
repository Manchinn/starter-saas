  ...
  <AppLayout>
    <div class="space-y-5">
      ...
      <div class="flex items-start gap-3 print:hidden">
        ...
        <div v-if="bn && !loading" class="flex items-center gap-2 flex-shrink-0">
          ...
          <button @click="onPrint" type="button"
            :title="t('erp.billingNotes.printDocument')"
      ...
      <template v-else>
        ...
        <!-- Document — billing note (ใบแจ้งยอด) -->
        ...
        <div v-can="'erp.accounting.edit'" v-if="forwardTransitions.length || cancelTransitions.length"
          ...
          <div>
            <p class="text-[11px] font-semibold text-[#9BA7B0] uppercase tracking-wider">{{ t('erp.billingNotes.nextAction') }}</p>
            <p class="text-[13px] text-[#637381] mt-0.5">{{ t('erp.billingNotes.nextActionHint') }}</p>
          </div>
...
const bn             = ref(null)
const totalInWords   = computed(() => bn.value ? numToWords(bn.value.total, locale.value, bn.value.currency) : '')
...
const customerTaxId = computed(() => bn.value?.customer?.taxId || '')
const fillerRows    = computed(() => Math.max(0, 8 - (bn.value?.lines?.length || 0)))
