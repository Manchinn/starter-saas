<template>
  <AppLayout>
    <div class="space-y-5">
      ...
      <PageHeader :title="t('erp.journals.new')" back-to="/erp/accounting/journals"
        :breadcrumb="[
        ...
        <template #actions>
          <HeaderSaveActions
            ...
            :save-label="t('erp.journals.newEntry')"
            :disabled="!canSave"
    ...
    <div class="sticky bottom-0 -mx-6 mt-6 px-6 py-3.5 bg-white/95 backdrop-blur border-t border-[#E2E8F0] shadow-[0_-4px_12px_rgba(15,23,42,0.05)] z-20
      ...
      <div class="flex items-center gap-2.5">
        ...
        <button @click="save" :disabled="!canSave || saving" type="button"
          ...
          {{ saving ? t('erp.common.creating') : t('erp.journals.newEntry') }}
        </button>
      </div>
    </div>
  </AppLayout>
...
// Chart-of-accounts grouped by accountType, sorted by code within each group.
...
// Debit/credit are mutually exclusive on a single line — typing into one
// clears the other so totals stay meaningful.
...
async function save() {
  ...
  saving.value = true
  try {
    const { data } = await api.post('/erp/accounting/journals', form.value)
    dirty.value = false
    const id = data?.data?.journal?.id
