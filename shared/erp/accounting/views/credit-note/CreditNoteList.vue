  ...
  <AppLayout>
    <div class="space-y-5">
      ...
      <div class="flex items-center justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold text-[#1C2434]">{{ t('erp.creditNotes.title') }}</h1>
          <p class="text-sm text-[#637381] mt-0.5">{{ total }} credit note{{ total !== 1 ? 's' : '' }}</p>
        </div>
        <div class="flex items-center gap-2">
          <KeyboardShortcuts :shortcuts="shortcuts" />
          <RouterLink
            v-can="'erp.accounting.edit'"
            to="/erp/billing/credit-notes/create"
            class="btn-primary">
            <PlusIcon class="w-4 h-4" />
            {{ t('erp.creditNotes.new') }}
          </RouterLink>
        </div>
      </div>

      <div class="bg-white border border-[#E2E8F0] shadow-sm overflow-hidden">
        <DataTable ref="dataTableRef" :columns="columns" :data="creditNotes" :loading="loading" :total="total"
          v-model:page="page" v-model:global-filter="search" :page-size="limit"
          :selected-row-index="selectedRowIndex"
          searchable :search-placeholder="t('erp.creditNotes.searchPh')">
          ...
          <template #empty>
            <div class="flex flex-col items-center gap-3 py-4">
              <div class="w-10 h-10 bg-[#F1F5F9] flex items-center justify-center">
                <ArrowTrendingDownIcon class="w-5 h-5 text-[#9BA7B0]" />
              </div>
              <div class="text-center">
                <p class="text-sm font-medium text-[#637381]">{{ t('erp.creditNotes.noFound') }}</p>
...
import { PlusIcon, EyeIcon, ArrowTrendingDownIcon } from '@heroicons/vue/24/outline'
...
const { selectedIndex: selectedRowIndex, shortcuts } = useListShortcuts({
  rows: creditNotes, page, totalPages,
  open:        r => router.push(`/erp/billing/credit-notes/${r.id}`),
  create:      () => router.push('/erp/billing/credit-notes/create'),
...
async function fetchCreditNotes() {
  loading.value = true
  try {
    const { data } = await api.get('/erp/billing/credit-notes', {
...
function onFilterChange() { page.value = 1; fetchCreditNotes() }

watch([page, search], fetchCreditNotes)
onMounted(fetchCreditNotes)

const STATUS_STYLE = {
  draft:     { badge: 'bg-[#F1F5F9] text-[#637381]', dot: 'bg-slate-400' },
  issued:    { badge: 'bg-green-50 text-green-700',   dot: 'bg-green-500' },
...
const columns = [
  columnHelper.accessor('refNo', {
    header: () => t('erp.creditNotes.colRefNo'),
  ...
  columnHelper.display({
    id: 'invoice',
    header: () => t('erp.creditNotes.colInvoice'),
    cell: info => {
  ...
  columnHelper.accessor('reason', {
    header: () => t('erp.creditNotes.colReason'),
  ...
  columnHelper.accessor('amount', {
    header: () => t('erp.creditNotes.colAmount'),
  ...
  columnHelper.display({
    ...
    cell: info => h('div', { class: 'flex items-center justify-end' }, [
      h(RouterLink, {
        to: `/erp/billing/credit-notes/${info.row.original.id}`,
