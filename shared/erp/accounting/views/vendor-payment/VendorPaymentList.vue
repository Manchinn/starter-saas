  ...
  <AppLayout>
    <div class="space-y-5">
      ...
      <div class="flex items-center justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold text-[#1C2434]">{{ t('erp.makePayments.title') }}</h1>
          <p class="text-sm text-[#637381] mt-0.5">{{ total }} payment{{ total !== 1 ? 's' : '' }}</p>
        </div>
        <div class="flex items-center gap-2">
          <KeyboardShortcuts :shortcuts="shortcuts" />
          <RouterLink v-can="'erp.accounting.edit'" to="/erp/billing/make-payments/create" class="btn-primary">
            <PlusIcon class="w-4 h-4" />
            {{ t('erp.makePayments.new') }}
          </RouterLink>
        </div>
      </div>

      <div class="bg-white border border-[#E2E8F0] shadow-sm overflow-hidden">
        <DataTable ref="dataTableRef" :columns="columns" :data="vendorPayments" :loading="loading" :total="total"
          v-model:page="page" v-model:global-filter="search" :page-size="limit"
          :selected-row-index="selectedRowIndex" row-clickable @row-click="openRow"
          searchable :search-placeholder="t('erp.makePayments.searchPh')">
          ...
          <template #empty>
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <div class="text-center">
                <p class="text-sm font-medium text-[#637381]">{{ t('erp.makePayments.noFound') }}</p>
...
const { selectedIndex: selectedRowIndex, shortcuts, open: openRow } = useListShortcuts({
  rows: vendorPayments, page, totalPages,
  open:        r => router.push(`/erp/billing/make-payments/${r.id}`),
  create:      () => router.push('/erp/billing/make-payments/create'),
...
async function fetchPayments() {
  loading.value = true
  try {
    const { data } = await api.get('/erp/billing/make-payments', {
      params: { page: page.value, limit, search: search.value, status: filterStatus.value || undefined },
    })
    vendorPayments.value = data.data.vendorPayments
...
const columns = [
  columnHelper.accessor('refNo', {
    header: () => t('erp.makePayments.colRefNo'),
    cell: info => h('span', { class: 'font-mono text-sm font-medium text-[#1C2434]' }, info.getValue()),
  }),
  columnHelper.display({
    id: 'vendor',
    header: () => t('erp.makePayments.colVendor'),
    cell: info => h('span', { class: 'text-sm text-[#374151]' }, info.row.original.vendor?.name || '—'),
  }),
  ...
  columnHelper.accessor('paymentMethod', {
    header: () => t('erp.makePayments.colMethod'),
    cell: info => h('span', { class: 'inline-flex items-center px-2 py-0.5 text-[10px] font-semibold bg-blue-50 text-blue-700' }, info.getValue() || '—'),
  }),
  columnHelper.accessor('amount', {
    header: () => t('erp.makePayments.colAmount'),
  ...
  columnHelper.accessor('status', {
    ...
    cell: info => {
      ...
      return h('span', { class: `inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold capitalize ${statusClass(s)}` }, [
        h('span', { class: `w-1.5 h-1.5 ${statusDot(s)}` }), s,
      ])
    },
  }),
  columnHelper.display({
    ...
    cell: info => h('div', { class: 'flex items-center justify-end' }, [
      h(RouterLink, {
        to: `/erp/billing/make-payments/${info.row.original.id}`,
