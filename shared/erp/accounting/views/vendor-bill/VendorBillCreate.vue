<template>
  <AppLayout>
    <div class="space-y-5">
      ...
      <PageHeader :title="t('erp.bills.new')" back-to="/erp/purchasing/bills"
        :breadcrumb="[
          ...
          <div class="px-6 py-5 grid grid-cols-1 lg:grid-cols-3 gap-x-6 gap-y-5">
            ...
            <div>
              ...
              <SearchSelect v-model="form.purchaseOrderId" :options="purchaseOrders" label-key="refNo"
                :placeholder="`— ${t('erp.bills.noPO')} —`" @change="onPurchaseOrderChange" />
            </div>
    ...
    <div class="sticky bottom-0 -mx-6 mt-6 px-6 py-3.5 bg-white/95 backdrop-blur border-t border-[#E2E8F0] shadow-[0_-4px_12px_rgba(15,23,42,0.05)] z-20
      ...
      <div class="flex items-center gap-2.5">
        ...
        <button @click="save" :disabled="!canSave || saving || savingDraft" type="button"
          :title="!canSave ? t('erp.bills.fillRequiredFields') : `${t('erp.bills.create')} (Ctrl+Shift+S)`"
          class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-bold
                 bg-primary-500 text-white hover:bg-primary-600 shadow-sm
                 disabled:opacity-50 disabled:cursor-not-allowed transition-colors">
          <ArrowPathIcon v-if="saving" class="w-4 h-4 animate-spin" />
          <CheckIcon v-else class="w-4 h-4" />
          {{ saving ? t('erp.common.creating') : t('erp.bills.create') }}
        </button>
      </div>
    </div>
...
const form = ref({
  ...
  vendorInvoiceNo: '', billDate: today, dueDate: '',
...
// Once a draft has been saved-without-redirect we have a bill id, so further
...
onMounted(async () => {
  const [vRes, pRes, poRes, grRes] = await Promise.allSettled([
...
// When a PO is selected, default vendor + items from that PO if the bill is
// still blank. Don't clobber existing edits.
async function onPurchaseOrderChange() {
  const id = form.value.purchaseOrderId
  if (!id) return
  try {
    const { data } = await api.get(`/erp/purchasing/orders/${id}`)
    const po = data.data.order
    if (!form.value.vendorId && po.vendorId) form.value.vendorId = po.vendorId
    if (!form.value.items.length && po.items?.length) {
...
async function save({ redirect = true } = {}) {
  ...
  try {
    ...
    let data
    if (createdId.value) {
      ({ data } = await api.put(`/erp/purchasing/bills/${createdId.value}`, payload))
    } else {
      ({ data } = await api.post('/erp/purchasing/bills', payload))
      createdId.value = data.data.bill.id
    }
    dirty.value = false
    if (redirect) {
      router.push(`/erp/purchasing/bills/${data.data.bill.id}`)
    } else {
