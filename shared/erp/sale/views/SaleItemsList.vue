  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between gap-4">
        ...
        <RouterLink to="/erp/sale-items/create" class="px-4 py-2 bg-primary-600 text-white text-sm rounded-lg hover:bg-primary-700 transition">
          + New Sale Item
        </RouterLink>
      </div>
      ...
      <div class="flex items-center gap-3 flex-wrap">
        ...
        />
        <select v-model="filterStatus" @change="() => { page = 1; loadItems() }" class="px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
        ...
        <table class="w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-200 text-left">
            <tr>
              ...
              <th class="px-5 py-3 font-medium text-gray-600">Product Master</th>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-else-if="!items.length">
              <td colspan="5" class="text-center py-12 text-gray-400">No sale items found.</td>
            </tr>
            <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50 transition">
              ...
              <td class="px-5 py-3 text-gray-600">
                <span v-if="item.product">{{ item.product.name }}<span v-if="item.product.sku" class="ml-1 text-xs text-gray-400 font-mono">({{ item.product.sku }})</span></span>
                <span v-else class="text-gray-300">—</span>
              </td>
              <td class="px-5 py-3">
                <span :class="item.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500'"
              ...
              <td class="px-5 py-3 text-right">
                <div class="flex items-center justify-end gap-2">
                  <RouterLink :to="`/erp/sale-items/${item.id}/edit`" class="text-xs px-3 py-1 border rounded hover:bg-gray-50">Edit</RouterLink>
                  <button @click="confirmDelete(item)" class="text-xs px-3 py-1 border border-red-200 text-red-500 rounded hover:bg-red-50">Delete</button>
                </div>
              </td>
            </tr>
          </tbody>
    ...
    <Teleport to="body">
      <div v-if="deleteModal.open" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
        <div class="bg-white rounded-xl shadow-xl w-full max-w-sm p-6 space-y-4">
          ...
          <p class="text-sm text-gray-600">Delete <span class="font-medium">{{ deleteModal.item?.name }}</span>? This cannot be undone.</p>
...
function onSearch() {
  ...
  searchTimeout = setTimeout(() => { page.value = 1; loadItems() }, 300)
}
...
async function doDelete() {
  ...
  deleteModal.error  = ''
  try {
    await api.delete(`/erp/sale-items/${deleteModal.item.id}`)
