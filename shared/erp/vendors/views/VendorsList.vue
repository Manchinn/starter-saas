              ...
              <div v-if="showFilters" class="border-b border-[#E2E8F0] bg-slate-50">
                <div class="px-5 py-4">
                  <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                    ...
                    <SearchSelectWithLabel v-model="filterType" :label="t('erp.vendors.colType')" :label-class="FILTER_LABEL" :options="typeOptions" :placeholder="t('common.all')" @change="onFilterChange" />
          ...
          <template #active-filters>
            <div v-if="activeFilterCount > 0" class="px-5 py-2.5 border-b border-[#E2E8F0] flex items-center gap-2 flex-wrap bg-primary-50/40">
              ...
              <span v-if="filterType" class="inline-flex items-center gap-1 pl-2.5 pr-1 py-1 bg-white border border-[#E2E8F0] text-xs font-medium text-[#374151]">
                {{ t('erp.vendors.colType') }}: <span class="font-semibold ml-0.5">{{ filterType === 'service_provider' ? t('erp.vendors.serviceProvider') : t('erp.vendors.supplier') }}</span>
          ...
          <template #empty>
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <div class="text-center">
                <p class="text-sm font-medium text-[#637381]">{{ t('erp.vendors.noFound') }}</p>
...
async function load() {
  ...
  try {
    const { data } = await api.get('/erp/vendors', {
      params: { page: page.value, limit, search: search.value, status: filterStatus.value, typeFilter: filterType.value, activeFrom: filterActiveFrom.value || undefined, activeTo: filterActiveTo.value || undefined },
    })
...
function clearFilters() { filterStatus.value = ''; filterType.value = ''; filterActiveFrom.value = ''; filterActiveTo.value = ''; page.value = 1; load() }
  ...
  columnHelper.display({
    ...
    cell: info => {
      ...
      return h('div', { class: 'flex flex-wrap gap-1' }, types.map(vType =>
        h('span', {
          key: vType,
          class: `inline-flex items-center px-2 py-0.5 text-xs font-medium ${vType === 'service_provider' ? 'bg-purple-50 text-purple-700' : 'bg-blue-50 text-blue-700'}`
