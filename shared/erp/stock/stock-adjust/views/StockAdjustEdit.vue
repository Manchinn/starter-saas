<template>
  <AppLayout>
    <div class="space-y-6">
      ...
      <PageHeader :title="t('erp.stockAdjust.edit')" :back-to="`/erp/stock-adjust/${route.params.id}`"
        :breadcrumb="[
          { label: t('erp.stockAdjust.title'), to: '/erp/stock-adjust' },
          { label: form.refNo || '…', to: `/erp/stock-adjust/${route.params.id}` },
          { label: t('common.edit') },
        ]">
        <template #badge>
        ...
        <div class="bg-white rounded-2xl border border-[#E2E8F0] shadow-card overflow-hidden">
          ...
          <div class="px-6 py-5 bg-[#F7F9FC] border-t border-[#E2E8F0] flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
              ...
              <RouterLink :to="`/erp/stock-adjust/${route.params.id}`"
...
onMounted(async () => {
  try {
    const [storesRes, adjRes] = await Promise.all([
      api.get('/erp/stock-adjust/stores-lookup'),
      api.get(`/erp/stock-adjust/${route.params.id}`),
    ])
    stores.value = storesRes.data.data.stores
    adjustReasons.value = await masterDataStore.getValues('adjustment-reasons')

    const adj = adjRes.data.data.adjustment
    if (!adj) { loadError.value = 'Adjustment not found'; return }
    if (adj.status !== 'draft') {
      loadError.value = 'Only draft adjustments can be edited. This one is already ' + adj.status + '.'
      return
    }
    ...
    initialStoreId = adj.storeId
    skipNextStoreWatch = true

    if (initialStoreId) {
      loadingStoreProducts.value = true
      try {
        const { data } = await api.get('/erp/stock-adjust/store-products', { params: { storeId: initialStoreId } })
...
async function save() {
  ...
  saving.value = true
  try {
    await api.put(`/erp/stock-adjust/${route.params.id}`, {
