    ...
    <div class="space-y-5">
      ...
      <div class="bg-white border border-[#E2E8F0] shadow-sm overflow-hidden">
        <DataTable ref="dataTableRef" :columns="columns" :data="rows" :loading="loading" :total="total"
          ...
          searchable :search-placeholder="t('erp.stockAdjust.searchPh')">
          ...
          <template #empty>
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <div class="text-center">
                <p class="text-sm font-medium text-[#637381]">{{ t('erp.stockAdjust.noFound') }}</p>
...
import {
  PlusIcon, EyeIcon, CheckCircleIcon, TrashIcon,
...
const { selectedIndex: selectedRowIndex, shortcuts } = useListShortcuts({
  ...
  open:        r => router.push(`/erp/stock-adjust/${r.id}`),
  create:      () => router.push('/erp/stock-adjust/create'),
...
async function load() {
  loading.value = true
  try {
    const { data } = await api.get('/erp/stock-adjust', {
      params: {
