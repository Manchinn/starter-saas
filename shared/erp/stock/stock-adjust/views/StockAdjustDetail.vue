...
const { shortcuts } = useDetailShortcuts({
  enabled: () => !loading.value && !!adj.value,
  canEdit: () => adj.value?.status === 'draft',
  edit:  () => router.push(`/erp/stock-adjust/${adj.value.id}/edit`),
  print: onPrint,
...
const fillerRows = computed(() => Math.max(0, 8 - (adj.value?.items?.length || 0)))
const signatures = computed(() => [
  t('erp.stockAdjust.docPreparedBy'),
  t('erp.stockAdjust.docApprovedBy'),
])
const stampLabel = computed(() => {
  const s = adj.value?.status
  if (s === 'draft')     return 'Draft'
  if (s === 'confirmed') return 'Confirmed'
  return ''
})
const stampClass = computed(() =>
  adj.value?.status === 'confirmed'
