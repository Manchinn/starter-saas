...
class StockRequestController extends BaseController {
  ...
  async create(req, res) {
    try {
      const request = await service.create({ ...req.body, userId: req.user?.id })
      return this.created(res, { request }, 'Stock Transfer created')
    } catch (err) {
      return this.fail(res, err.message, err.status || 400)
    }
  }
  ...
  async confirm(req, res) {
    try {
      ...
      return this.ok(res, { request }, 'Stock Transfer confirmed')
    } catch (err) {
      return this.fail(res, err.message, err.status || 400)
    }
  }
  ...
  async remove(req, res) {
    try {
      ...
      return this.ok(res, null, 'Stock Transfer deleted')
    } catch (err) {
      return this.fail(res, err.message, err.status || 400)
    }
  }
