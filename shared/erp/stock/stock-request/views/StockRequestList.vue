  ...
  <AppLayout>
    <div class="space-y-5">
      ...
      <div class="flex items-center justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold text-gray-900">Stock Transfers</h1>
          <p class="text-sm text-gray-500 mt-0.5">{{ total }} record{{ total !== 1 ? 's' : '' }}</p>
        </div>
        <RouterLink to="/erp/stock-request/create"
      ...
      <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div class="px-5 py-3 border-b border-gray-100 flex items-center gap-3">
          <div class="relative flex-1 min-w-48 max-w-64">
            ...
            <input v-model="search" @input="onSearch" type="search" placeholder="Search ref no, notes…"
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-gray-50 border-b border-gray-100 text-left">
              ...
              <th class="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">From Store</th>
              <th class="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">To Store</th>
          ...
          <tbody class="divide-y divide-gray-50">
            ...
            <tr v-for="row in rows" :key="row.id" class="hover:bg-gray-50 transition-colors group">
              ...
              <td class="px-5 py-3.5 font-medium text-gray-700">{{ row.fromStore?.name || '—' }}</td>
              <td class="px-5 py-3.5 font-medium text-gray-700">{{ row.toStore?.name || '—' }}</td>
              ...
              <td class="px-5 py-3.5">
                <div class="flex items-center justify-end gap-1.5 transition-opacity">
                  <RouterLink :to="`/erp/stock-request/${row.id}`"
...
import { PlusIcon, MagnifyingGlassIcon, EyeIcon, CheckCircleIcon, TrashIcon, ChevronLeftIcon, ChevronRightIcon, ArrowsRightLeftIcon } from '@heroicons/vue/24/outline'
...
async function load() {
  loading.value = true
  try {
    const { data } = await api.get('/erp/stock-request', { params: { page: page.value, limit, search: search.value } })
