  ...
  <AppLayout>
    <div class="space-y-5">
      ...
      <div class="flex items-center justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold text-[#1C2434]">{{ t('erp.stockTransfer.title') }}</h1>
          <p class="text-sm text-[#637381] mt-0.5">{{ total }} record{{ total !== 1 ? 's' : '' }}</p>
        </div>
        <div class="flex items-center gap-2">
          <KeyboardShortcuts :shortcuts="shortcuts" />
          <RouterLink to="/erp/stock-request/create" class="btn-primary">
            <PlusIcon class="w-4 h-4" />
            {{ t('erp.stockTransfer.new') }}
          </RouterLink>
        </div>
      </div>
      ...
      <div class="bg-white border border-[#E2E8F0] shadow-sm overflow-hidden">
        <DataTable ref="dataTableRef" :columns="columns" :data="rows" :loading="loading" :total="total"
          ...
          searchable :search-placeholder="t('erp.stockTransfer.searchPh')">
          ...
          <template #empty>
            <div class="flex flex-col items-center gap-3 py-4">
              ...
              <div class="text-center">
                <p class="text-sm font-medium text-[#637381]">{{ t('erp.stockTransfer.noFound') }}</p>
...
import {
  PlusIcon, EyeIcon, CheckCircleIcon, TrashIcon, ArrowsRightLeftIcon,
...
const { selectedIndex: selectedRowIndex, shortcuts, open: openRow } = useListShortcuts({
  ...
  open:        r => router.push(`/erp/stock-request/${r.id}`),
  create:      () => router.push('/erp/stock-request/create'),
  remove:      r => confirmDelete(r),
  focusSearch: () => dataTableRef.value?.focusSearch(),
  newLabel: 'New stock request',
})
...
const columns = [
  ch.accessor('refNo', {
    header: () => t('erp.stockTransfer.colRefNo'),
    cell: info => h('span', { class: 'font-mono font-medium text-[#1C2434]' }, info.getValue()),
  }),
  ch.accessor('date', {
    header: () => t('erp.stockTransfer.colDate'),
    cell: info => h('span', { class: 'text-[#637381] text-xs' }, fmtDate(info.getValue())),
  }),
  ch.accessor('fromStore', {
    header: () => t('erp.stockTransfer.colFromStore'),
    cell: info => h('span', { class: 'font-medium text-[#374151]' }, info.getValue()?.name || '—'),
  }),
  ch.accessor('toStore', {
    header: () => t('erp.stockTransfer.colToStore'),
    cell: info => h('span', { class: 'font-medium text-[#374151]' }, info.getValue()?.name || '—'),
  }),
  ch.accessor('status', {
    header: () => t('erp.stockTransfer.colStatus'),
    cell: info => {
      const s = info.getValue()
      const confirmed = s === 'confirmed'
      return h('span', {
        class: `inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold capitalize ${confirmed ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'}`,
      }, [
        h('span', { class: `w-1.5 h-1.5 ${confirmed ? 'bg-green-500' : 'bg-amber-500'}` }),
        s,
      ])
    },
  }),
    ...
    cell: ({ row }) => {
      ...
      return h('div', { class: 'flex items-center justify-end gap-1.5' }, [
        ...
        r.status === 'draft' && h('button', {
          ...
          class: 'p-1.5 text-[#9BA7B0] hover:text-green-700 hover:bg-green-50 transition-colors disabled:opacity-40',
          title: 'Approve',
...
async function load() {
  loading.value = true
  try {
    const { data } = await api.get('/erp/stock-request', {
      params: {
...
async function approveRow(row) {
  if (!confirm(`Approve ${row.refNo}? Stock will be transferred and this cannot be undone.`)) return
  approvingId.value = row.id
  try { await api.post(`/erp/stock-request/${row.id}/confirm`); load() }
...
async function confirmDelete(row) {
  ...
  try { await api.delete(`/erp/stock-request/${row.id}`); load() }
