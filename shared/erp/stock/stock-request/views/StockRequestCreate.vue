    ...
    <div class="space-y-6">
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-6">
        <div class="flex items-center justify-between mb-4">
          ...
          <button @click="addRow" :disabled="!form.fromStoreId || (storeProducts.length > 0 && allUsedIds.length >= storeProducts.length)"
        ...
        <!-- No store selected -->
        <div v-if="!form.fromStoreId" class="py-10 text-center text-sm text-gray-400">
          Select a <span class="font-medium text-gray-600">From Store</span> above to load available products.
        </div>
        ...
        <!-- No stock in store -->
        <div v-else-if="!storeProducts.length" class="py-10 text-center text-sm text-gray-400">
          No products with stock found in the selected store.
        </div>
        ...
        <table v-else class="w-full text-sm">
          <thead class="bg-gray-50 border-b border-gray-200 text-left">
            <tr>
              ...
              <th class="px-3 py-2 font-medium text-gray-600 w-32 text-right">Available</th>
              <th class="px-3 py-2 font-medium text-gray-600 w-32">Transfer Qty <span class="text-red-500">*</span></th>
          ...
          <tbody class="divide-y divide-gray-100">
            ...
            <tr v-for="(item, i) in items" :key="i" :class="item.qty > availableStock(item.productId) ? 'bg-red-50' : ''">
              <td class="px-3 py-2">
              ...
              <td class="px-3 py-2">
                <input v-model.number="item.qty" type="number" min="0.01" step="0.01" placeholder="0"
                  ...
                  :class="item.qty > availableStock(item.productId) ? 'border-red-400 text-red-600' : ''" />
              </td>
              <td class="px-3 py-2">
...
const stores              = ref([])
const storeProducts       = ref([])   // products with stock in the selected from-store
...
// Reload available products whenever the from-store changes
watch(() => form.value.fromStoreId, async (storeId) => {
  items.value = []          // clear items — product list is now different
  storeProducts.value = []
  if (!storeId) return
