    ...
    <div class="space-y-5">
      ...
      <template v-else>
        ...
        <article class="relative mx-auto bg-white border border-[#E2E8F0] shadow-card max-w-[860px] w-full
          ...
          <header class="px-10 pt-10 pb-6 flex items-start justify-between gap-8 border-b border-dashed border-[#E2E8F0]">
            ...
            <div class="text-right flex-shrink-0">
              <h2 class="text-[22px] font-extrabold tracking-[0.18em] text-[#1C2434] uppercase">
                {{ t('erp.stockTransfer.documentTitle') }}
              </h2>
          ...
          <!-- From / To stores -->
          <section class="px-10 py-6 grid grid-cols-1 sm:grid-cols-[1fr_auto_1fr] gap-x-6 gap-y-4 items-center border-b border-dashed border-[#E2E8F0]">
            <div>
              <p class="text-[10px] font-bold text-[#9BA7B0] uppercase tracking-[0.15em] mb-1.5">
                {{ t('erp.stockTransfer.fromStore') }}
              </p>
              <p class="text-[14px] font-bold text-[#1C2434]">{{ req.fromStore?.name || '—' }}</p>
              <p v-if="req.fromStore?.code" class="text-[11px] text-[#637381] font-mono mt-0.5">{{ req.fromStore.code }}</p>
            </div>
            <div class="flex items-center justify-center">
              <div class="w-10 h-10 bg-violet-50 border border-violet-200 flex items-center justify-center">
                <ArrowRightIcon class="w-5 h-5 text-violet-600" />
              </div>
            </div>
            <div class="text-right sm:text-left">
              <p class="text-[10px] font-bold text-[#9BA7B0] uppercase tracking-[0.15em] mb-1.5">
                {{ t('erp.stockTransfer.toStore') }}
              </p>
              <p class="text-[14px] font-bold text-[#1C2434]">{{ req.toStore?.name || '—' }}</p>
              <p v-if="req.toStore?.code" class="text-[11px] text-[#637381] font-mono mt-0.5">{{ req.toStore.code }}</p>
            </div>
          </section>
            ...
            <table class="w-full text-[12px]">
              <thead>
                <tr class="border-b-2 border-[#1C2434] text-[10px] font-bold text-[#1C2434] uppercase tracking-wider">
                  ...
                  <th class="py-2.5 text-left w-28">{{ t('erp.stockTransfer.colSku') }}</th>
                  <th class="py-2.5 text-left">{{ t('erp.common.product') }}</th>
                  <th class="py-2.5 text-right w-20">{{ t('erp.stockTransfer.colQty') }}</th>
              ...
              <tbody>
                <tr v-for="(item, idx) in (req.items || [])" :key="item.id" class="border-b border-[#F1F5F9]">
                  ...
                  <td class="py-2.5 align-top text-right font-semibold text-violet-700 tabular-nums">
                    {{ Number(item.qty) }}
                  </td>
          ...
          <section class="px-10 pb-6 flex justify-end">
            <dl class="w-full sm:w-72 text-[12px] space-y-1.5">
              <div class="flex items-center justify-between">
                ...
                <dd class="font-semibold text-[#1C2434] tabular-nums">{{ (req.items || []).length }}</dd>
              </div>
              <div class="flex items-center justify-between pt-2 mt-1 border-t-2 border-[#1C2434]">
                <dt class="text-[11px] font-bold text-[#1C2434] uppercase tracking-wider">{{ t('erp.stockTransfer.totalTransferQty') }}</dt>
                <dd class="text-[16px] font-extrabold text-violet-700 tabular-nums">{{ totalQty }}</dd>
              </div>
            </dl>
          </section>
...
import {
  ArrowLeftIcon, ChevronRightIcon, ArrowRightIcon, CheckIcon, ArrowPathIcon,
...
const pageShortcuts = computed(() => [
  ...(req.value?.status === 'draft' ? [{ key: 'E', label: 'Edit' }] : []),
...
function onKeydown(e) {
  if (isTyping() || loading.value || !req.value) return
  if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'p') {
    e.preventDefault(); onPrint()
  } else if (!e.ctrlKey && !e.metaKey && !e.altKey && e.key === 'e') {
    if (req.value.status === 'draft') router.push(`/erp/stock-request/${req.value.id}/edit`)
