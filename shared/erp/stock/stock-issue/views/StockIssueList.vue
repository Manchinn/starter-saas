      ...
      <div class="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden">
        <div class="px-5 py-3 border-b border-[#E2E8F0] flex items-center gap-3">
          <div class="relative flex-1 min-w-48 max-w-64">
            ...
            <input v-model="search" @input="onSearch" type="search" :placeholder="t('erp.stockIssue.searchPh')" class="input pl-9" />
          </div>
        </div>
        ...
        <DataTable :columns="columns" :data="rows" :loading="loading" :total="total" v-model:page="page" :page-size="limit">
          <template #empty>
            <div class="flex flex-col items-center gap-2">
              ...
              <p class="text-sm text-[#9BA7B0] font-medium">{{ t('erp.stockIssue.noFound') }}</p>
            </div>
          </template>
        </DataTable>
      </div>
...
import { PlusIcon, MagnifyingGlassIcon, EyeIcon, CheckCircleIcon, TrashIcon, ArrowUpTrayIcon } from '@heroicons/vue/24/outline'
...
async function load() {
  loading.value = true
  try {
    const { data } = await api.get('/erp/stock-issue', { params: { page: page.value, limit, search: search.value } })
