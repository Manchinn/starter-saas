  ...
  <AppLayout>
    <div class="space-y-5">
      ...
      <div class="flex items-center justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold text-[#1C2434]">{{ t('erp.stockIssue.title') }}</h1>
          <p class="text-sm text-[#637381] mt-0.5">{{ total }} record{{ total !== 1 ? 's' : '' }}</p>
        </div>
        <div class="flex items-center gap-2">
          <KeyboardShortcuts :shortcuts="shortcuts" />
          <RouterLink to="/erp/stock-issue/create" class="btn-primary">
            <PlusIcon class="w-4 h-4" />
            {{ t('erp.stockIssue.new') }}
          </RouterLink>
        </div>
      </div>
          ...
          <template #empty>
            <div class="flex flex-col items-center gap-3 py-4">
              <div class="w-10 h-10 bg-[#F1F5F9] flex items-center justify-center">
                <ArrowUpTrayIcon class="w-5 h-5 text-[#9BA7B0]" />
              </div>
              <div class="text-center">
                <p class="text-sm font-medium text-[#637381]">{{ t('erp.stockIssue.noFound') }}</p>
...
const { selectedIndex: selectedRowIndex, shortcuts } = useListShortcuts({
  ...
  open:        r => router.push(`/erp/stock-issue/${r.id}`),
  create:      () => router.push('/erp/stock-issue/create'),
...
async function load() {
  loading.value = true
  try {
    const { data } = await api.get('/erp/stock-issue', {
      params: {
...
async function approveRow(row) {
  if (!confirm(`Confirm ${row.refNo}? Stock will be deducted and this cannot be undone.`)) return
  approvingId.value = row.id
  try { await api.post(`/erp/stock-issue/${row.id}/confirm`); load() }
...
async function confirmDelete(row) {
  ...
  try { await api.delete(`/erp/stock-issue/${row.id}`); load() }
