  ...
  <AppLayout>
    <div class="space-y-5">
      ...
      <div class="flex items-center justify-between gap-4">
        <div>
          <h1 class="text-xl font-semibold text-gray-900">Stock Issues</h1>
          <p class="text-sm text-gray-500 mt-0.5">{{ total }} record{{ total !== 1 ? 's' : '' }}</p>
        </div>
        <RouterLink to="/erp/stock-issue/create"
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="bg-gray-50 border-b border-gray-100 text-left">
              ...
              <th class="px-5 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Reason</th>
          ...
          <tbody class="divide-y divide-gray-50">
              ...
              <td colspan="6" class="text-center py-16">
                <div class="flex flex-col items-center gap-2">
                  <div class="w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center">
                    <ArrowUpTrayIcon class="w-5 h-5 text-gray-400" />
                  </div>
            ...
            <tr v-for="row in rows" :key="row.id" class="hover:bg-gray-50 transition-colors group">
              ...
              <td class="px-5 py-3.5 text-gray-600">{{ row.reason || '—' }}</td>
              ...
              <td class="px-5 py-3.5">
                <div class="flex items-center justify-end gap-1.5 transition-opacity">
                  <RouterLink :to="`/erp/stock-issue/${row.id}`"
...
import { PlusIcon, MagnifyingGlassIcon, EyeIcon, CheckCircleIcon, TrashIcon, ChevronLeftIcon, ChevronRightIcon, ArrowUpTrayIcon } from '@heroicons/vue/24/outline'
