  ...
  <AppLayout>
    <div class="space-y-6">
      <div class="flex items-center justify-between gap-4">
        <h1 class="text-2xl font-bold text-gray-900">Stock Issues</h1>
            ...
            <tr v-for="row in rows" :key="row.id" class="hover:bg-gray-50 transition">
              ...
              <td class="px-5 py-3 text-right">
                <div class="flex items-center justify-end gap-2">
                  <RouterLink :to="`/erp/stock-issue/${row.id}`" class="text-xs px-3 py-1 border rounded hover:bg-gray-50">View</RouterLink>
...
async function approveRow(row) {
  ...
  try {
    await api.post(`/erp/stock-issue/${row.id}/confirm`)
    load()
  } catch (err) {
...
async function confirmDelete(row) {
  ...
  try {
    await api.delete(`/erp/stock-issue/${row.id}`)
    load()
  } catch (err) {
