...
const pageShortcuts = computed(() => [
  ...(issue.value?.status === 'draft' ? [{ key: 'E', label: 'Edit' }] : []),
...
function onKeydown(e) {
  if (isTyping() || loading.value || !issue.value) return
  if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'p') {
    e.preventDefault(); onPrint()
  } else if (!e.ctrlKey && !e.metaKey && !e.altKey && e.key === 'e') {
    if (issue.value.status === 'draft') router.push(`/erp/stock-issue/${issue.value.id}/edit`)
