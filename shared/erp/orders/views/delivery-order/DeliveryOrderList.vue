    ...
    <div class="space-y-5">
      ...
      <div class="bg-white rounded-2xl border border-[#E2E8F0] shadow-sm overflow-hidden">
        ...
        <div class="px-5 py-3 border-b border-[#E2E8F0] flex items-center gap-3">
          ...
          <select v-model="filterStatus" @change="onFilterChange" class="input text-sm w-44">
            ...
            <option value="shipped">{{ t('erp.deliveryOrders.statusShipped') }}</option>
            <option value="delivered">{{ t('erp.deliveryOrders.statusDelivered') }}</option>
...
const columns = [
  ...
  columnHelper.accessor('status', {
    ...
    cell: info => {
      ...
      return h('span', {
        class: `inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold capitalize ${badgeClass(s)}`
      }, [
        h('span', { class: `w-1.5 h-1.5 rounded-full ${dotClass(s)}` }),
        s,
      ])
    },
  }),
