        ...
        <DataTable :columns="columns" :data="employees" :loading="loading" :total="total"
          ...
          <template #active-filters>
            <div v-if="activeFilterCount > 0" class="px-5 py-2.5 border-b border-[#E2E8F0] flex items-center gap-2 flex-wrap bg-primary-50/40">
              ...
              <span v-if="filterDeptId" class="inline-flex items-center gap-1 pl-2.5 pr-1 py-1 bg-white border border-[#E2E8F0] rounded-full text-xs font-medium text-[#374151]">
                {{ t('erp.employees.colDepartment') }}: <span class="font-semibold ml-0.5">{{ deptLabel }}</span>
                <button @click="filterDeptId = ''; onFilterChange()" class="ml-1 p-0.5 text-[#9BA7B0] hover:text-red-500 rounded-full transition-colors">
                  <XMarkIcon class="w-3 h-3" />
                </button>
              </span>
