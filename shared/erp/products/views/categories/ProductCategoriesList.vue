...
const SHORTCUTS = [
  ...
  { key: 'Shift+C',  label: 'New category' },
...
function onKeydown(e) {
  ...
  if (e.key === 'ArrowDown') {
    e.preventDefault()
    selectedRowIndex.value = Math.min(selectedRowIndex.value + 1, categories.value.length - 1)
    ...
    const cat = categories.value[selectedRowIndex.value]
    if (cat) router.push(`/erp/product-categories/${cat.id}/edit`)
  } else if (e.shiftKey && e.key === 'C') {
    e.preventDefault()
    router.push('/erp/product-categories/create')
  ...
  columnHelper.display({
    ...
    cell: info => h('div', { class: 'flex items-center justify-end gap-1' }, [
      h(RouterLink, {
        ...
        class: 'p-1.5 text-[#9BA7B0] hover:text-primary-500 hover:bg-primary-50 transition-colors inline-flex items-center',
        title: t('common.edit'),
