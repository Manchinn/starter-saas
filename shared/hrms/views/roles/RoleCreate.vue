  ...
  <AppLayout>
    <div class="space-y-6">
      ...
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-xl font-semibold text-[#1C2434]">{{ t('hrms.roles.newTitle') }}</h1>
          <p class="text-sm text-[#637381] mt-0.5">{{ t('hrms.roles.newDesc') }}</p>
        </div>
      </div>
      ...
      <div class="bg-white border border-[#E2E8F0] shadow-sm overflow-hidden">
        ...
        <div class="px-6 py-5 space-y-5">
          <div class="grid grid-cols-2 gap-5">
            ...
            <FormField v-model="form.name" name="name" :label="t('common.name')" required :errors="fieldErrors" />

            <FormField
              v-model="form.slug" name="slug" :label="t('common.slug')"
              :placeholder="t('hrms.roles.slugPh')" :errors="fieldErrors" input-class="font-mono"
            >
              <template #label>
                {{ t('common.slug') }}
                <span class="text-[#9BA7B0] font-normal normal-case ml-1">{{ t('hrms.roles.slugHint') }}</span>
              </template>
            </FormField>
      ...
      <div class="flex justify-end gap-3">
        ...
        <button @click="save" :disabled="saving" class="btn-primary">
          {{ saving ? t('common.saving') : t('hrms.roles.create') }}
        </button>
      </div>
...
const form = reactive({ name: '', slug: '', description: '', color: '#6366f1' })
