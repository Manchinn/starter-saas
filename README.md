...
The client display name is built from `VITE_APP_NAME` (default: `MakeIt`). Set it in `client/.env` before running a production build; see [`client/.env.example`](client/.env.example). The server-side `APP_NAME` remains the source for mail subjects and mailer footers.
